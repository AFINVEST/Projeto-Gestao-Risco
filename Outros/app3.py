...
# Cria um header fixo no topo do corpo principal
st.markdown(
    """
    <div class="custom-header">
        <h1>Portifólio de Análise de Risco</h1>
    </div>
    """,
...
if st.button("Rodar Otimização MVaR"):
    ...
    if n < 2:
        ...
        df_port = df_portifolio_analise
        vol_each = df_port.std()
...
def calculate_portfolioVaR(df_returns, assets, weights):
    ...
    var_total = np.sqrt(var_individual + var_cor)
    return var_total


var_tot = calculate_portfolioVaR(df_returns, assets, quantidades)
