...
SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImNseXlobGlidm1uYXZkcGZzYnRyIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc3MDIyOTk0NCwiZXhwIjoyMDg1ODA1OTQ0fQ.SxrA8VqFX_PUzJhDOJ3PdZ2SFgH4CnYZBzb6LEjUhfc'
import os as _os_env
_os_env.environ.setdefault("SUPABASE_URL", SUPABASE_URL)
_os_env.environ.setdefault("SUPABASE_KEY", SUPABASE_KEY)
        ...
        for idx, row in df_contratos.iterrows():
            if idx in fundos:
                ...
                if vp_soma == 0:
                    ...
                    sum_row = pd.DataFrame({
                        'DIV01': [f"R${abs(df_divone_juros_nominais.iloc[0]) + abs(df_divone_juros_real.iloc[0]) + abs(df_divone_juros_externo_certo.iloc[0]) + abs(df_divone_dolar.iloc[0]):,.2f}"] if lista_dolar else [f"R${abs(df_divone_juros_nominais.iloc[0]) + abs(df_divone_juros_real.iloc[0]) + abs(df_divone_juros_externo_certo.iloc[0]):,.2f}"],
...
# ── alias antigo -> novo (idempotente) — patch fase2c ───────────────────────
import re as _re_bf
...
#
    ...
    with tab_orcamento:
        ...
        # Le dv01_por_ativo (JSON) direto do snapshot_diario — bate 100% com o summary
        dv01_asset_bps_dict = {}
        try:
        ...
        if df_hist_cv.empty:
            ...
            with colll2:
              if False:  # HIDDEN Fase 3: substituido pelo novo chart historico CoVaR
                ...
                df_hist_cv_positive = df_hist_cv_positive.drop(pd.to_datetime(['2025-11-11', '2025-11-12']), axis=0)
...
def main_page():
    ...
    if opti == "Adicionar Ativos":
        ...
        if len(assets) > 0:
            ...
            sum_row = pd.DataFrame({
                'DIV01': [f"R${df_divone_juros_nominais.iloc[0] + df_divone_juros_real.iloc[0] + df_divone_juros_externo + df_divone_dolar:,.2f}" if lista_juros_externo else f"R${df_divone_juros_nominais.iloc[0] + df_divone_juros_real.iloc[0] + df_divone_juros_externo.iloc[0] + df_divone_dolar:,.2f}"],
