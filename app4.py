...
def second_page():
    ...
    for ativo in ativos:
        data.append({
            "Ativo": ativo,
            "Quantidade": posicoes[ativo],
            "Preço Informado": precos_user[ativo],
            "Exposição R$": posicoes[ativo] * precos_user[ativo]
        })
    df_resumo = pd.DataFrame(data)
    st.write("## Resumo das posições com preços informados:")
    st.table(df_resumo.style.format({"Exposição R$": "R${:,.2f}"}))
...
def main_page():
    ...
    if len(assets) > 0:
        ...
        # VaR
        var_port = var_not_parametric(df_returns_portifolio['Portifolio']).abs()
        var_port_dinheiro = vp_soma * var_port
        ...
        with col2:
            ...
            if colunas_selecionadas:
                ...
                sum_row = tabela_filtrada.select_dtypes(include='number').sum()
                # Forçar a coerência dos nomes das colunas
                for col_name in ['Beta', 'MVar(R$)', 'CoVaR(R$)', 'CoVaR(%)', 'Var', '% do Risco Total']:
                    if col_name not in sum_row.index:
                        sum_row[col_name] = 0
                sum_row = sum_row.to_frame().T
                sum_row.index = ['Total']
                tabela_filtrada_com_soma = pd.concat([tabela_filtrada, sum_row])
