...
def salvar_chave(valor: bool):
    # Salva como string ("True" ou "False")
    Path("chave.txt").write_text(str(valor))


def carregar_chave() -> bool:
    # Converte para booleano
    return Path("chave.txt").read_text().strip() == "True"


def atualziar_quantidade(chave, df_precos_ajustados, quantidade_inicial):
    chave2 = carregar_chave()
    if chave2 == False:
        if chave == False:
            qtd_input = []
            quantidade_nomes = {}
            ...
            return quantidade_nomes, qtd_input, precos_user
            ...
            df_quant = pd.DataFrame(quantidade_nomes, index=False)
            df_quant.to_csv('quantidades.csv', index=False)
            return precos_user
    elif chave2 == True:
        if chave == False:
            quantidade_inicial = pd.read_csv('quantidades.csv')
            quantidade_inicial = quantidade_inicial.to_dict()
            qtd_input = []
            quantidade_nomes = {}
            precos_user = {}
            for col in df_precos_ajustados.index:
...
def main_page():
    ...
    if opti == "Adicionar Ativos":
        ...
        if len(assets) > 0:
            ...
            quantidade_nomes, qtd_input, precos_user = atualziar_quantidade(
                chave, df_precos_ajustados, quantidade_inicial)
            ...
            precos_user = atualziar_quantidade(chave, df_precos_ajustados,
                                               quantidade_nova_assets)
            ...
            if quantidade_nova_assets:
                st.session_state.quantidade_inicial.update(
                    quantidade_nova_assets)
...
def main_page():
    ...
    if opti == "Adicionar Ativos":
        ...
        default_or_not = st.sidebar.checkbox(
            "Usar ativos portifólio", value=False)
        ...
        if len(assets) > 0:
            ...
            if "quantidade_inicial" not in st.session_state:
                st.session_state["quantidade_inicial"] = quantidade_inicial2

            # Exibindo os valores para depuração
            st.write("Quantidade inicial: ",
                     st.session_state["quantidade_inicial"])

            # Inicializando uma lista de quantidades e um dicionário para preços
            qtd_input = []
            precos_user = {}

            # Loop para preencher os inputs
            for col in df_precos_ajustados.index:
                # Verifica se existe valor em st.session_state["quantidade_inicial"] para a ação 'col'
                default_qty = st.session_state["quantidade_inicial"].get(
                    col, 0)

                # Exibe o número de contratos com um input
                val = st.sidebar.number_input(
                    f"Quantidade para {col}:",
                    min_value=-10000,
                    value=default_qty,  # O valor inicial vem do session_state
                    step=1
                )

                # Atualiza st.session_state["quantidade_inicial"] com o novo valor, sem duplicação
                st.session_state["quantidade_inicial"][col] = val
                ...
                # Armazena os preços fornecidos pelo usuário
                precos_user[col] = preco

                # Adiciona o valor de quantidade ao vetor de entradas
                qtd_input.append(val)

                # Atualiza o dicionário de quantidades
                quantidade_nomes[col] = val

            # Exibe os resultados de quantidade_inicial atualizada
            st.write("Quantidade inicial atualizada: ",
                     st.session_state["quantidade_inicial"])
            ...
            quantidade_antiga_assets = {}
            for col in filtered_df.columns:
                if 'Contratos' in col:
                    quantidade_antiga_assets[col.split(
                        ' ')[1]] = filtered_df[col].sum()
            ...
            if quantidade_nova_assets != quantidade_antiga_assets:
                for asset, qtd_nova in quantidade_nova_assets.items():
                    st.session_state["quantidade_inicial"][asset] = int(
                        qtd_nova)
                    st.write(
                        f"Quantidade de {asset} atualizada para {int(qtd_nova)}.")
