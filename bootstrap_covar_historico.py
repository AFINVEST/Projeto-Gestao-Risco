"""bootstrap_covar_historico.py - popula covar_por_classe_pct em todos snapshots"""
import os, sys, time
...
print("[covar-hist] carregando datas snapshot...")
snaps = []
offset = 0
while True:
    ...
    offset += 1000
print(f"[covar-hist] {len(snaps)} datas")
...
for i, row in enumerate(snaps):
    d = row["Data"]
    try:
        res = calcular_covar_completo(
            data_ref=pd.Timestamp(d), basefundos=bf, janela_dias=756,
        )
        if "erro" in res:
            sem += 1; continue
...
print(f"[covar-hist] OK - {ok} atualizados, {sem} sem dados")
print(f"[covar-hist] tempo total: {(time.time()-t0)/60:.1f}min")
