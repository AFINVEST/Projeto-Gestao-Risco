...
def montar_html(snapshots_mes, snapshots_all, atrib_items, rent_hist, ind_hist,
    ...
    if not snapshots_mes:
        return f"Book Juros - Consolidado {label_mes} - sem dados", "<p>Sem snapshots no periodo.</p>", {}
    ...
    chart_waterfall = _svg_waterfall(atrib_items, largura=800, altura=340,
                                       titulo=f"Atribuição de Performance {label_mes}")
    ...
    chart_cota_hist = _svg_line_chart(datas_h,
        ...
        largura=800, altura=200, titulo="Cota vs CDI - Histórico total (retorno acumulado)",
        cores=["#1565c0", "#7f7f7f"], y_fmt="pct")
...
<h1>Book Juros - Consolidado {label_mes}</h1>
...
PL médio do mês: {_fmt_reais(pl_medio)}.
    ...
    assunto = f"Book Juros - Consolidado {label_mes}"
    dados = {
...
Book Juros - Consolidado {dados['label']}
</h2>
...
<b>Relatório completo com atribuição, rentabilidade histórica e índices:</b><br>
